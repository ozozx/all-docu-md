...
- WORKSPACE is a valid workspace identifier (see [[Dispatchers#Workspace|Dispatchers->Workspaces]]). This field is mandatory. This _can be_ a workspace selector, but please note workspace selectors can only match _existing_ workspaces.
...
| on_created_empty | A command to be executed once a workspace is created empty (i.e. not created by moving a window to it). See the [[Dispatchers#Executing with rules\|command syntax]] | string |
