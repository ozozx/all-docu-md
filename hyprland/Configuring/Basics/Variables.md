...
| allow_tearing | master switch for allowing tearing to occur. See [[Tearing\|the Tearing page]]. | bool | `false` |
...
> [!NOTE]
> _[[Animations|More about Animations]]._
...
| scroll_factor | Multiplier added to scroll movement for external mice. Note that there is a separate setting for [[#Touchpad\|touchpad scroll factor]].  | float | `1.0` |
...
>
> For switchable keyboard configurations, take a look at [[Binds#Switchable keyboard layouts|the binds page entry]].
...
| name | description | type | default |
| --- | --- | --- | --- |
| transform | Transform the input from touchdevices. The possible transformations are the same as [[Monitors#Rotating\|those of the monitors]]. `-1` means it's unset. | int | `-1` |
...
| name | description | type | default |
| --- | --- | --- | --- |
| transform | transform the input from tablets. The possible transformations are the same as [[Monitors#Rotating\|those of the monitors]]. `-1` means it's unset. | int | `-1` |
...
| eraser_button_override | Set a button to be button event when eraser_button_mode is set to `1`. Has to be an int, cannot be a string. Must be a valid button (e.g. BTN_STYLUS) excluding fake buttons (e.g. BTN_TOOL\_\*) and keys (KEY\_\*). Check wev if you have any doubts regarding the ID. `0` means default. | int | 0 |
...
| name | description | type | default |
| --- | --- | --- | --- |
| auto_group | whether new windows will be automatically grouped into the focused unlocked group. Note: if you want to disable auto_group only for specific windows, use [[Window Rules#`group` window rule options\|the "group barred" window rule]] instead. | bool | `true` |
...
| render_unfocused_fps | the maximum limit for render_unfocused windows' fps in the background (see also [[Window Rules#Dynamic effects\|Window-Rules]] - `render_unfocused`)| int | `15` |
...
| create_abstract_socket | Create the [[XWayland#Abstract Unix domain socket\|abstract Unix domain socket]] for XWayland connections. (XWayland restart is required for changes to take effect; Linux only) | bool | `false` |
...
| enforce_permissions | whether to enable [[Permissions\|permission control]].                  | bool | `false` |
