...
This problem is mitigated by the <code>[[hyprland/Configuring/Basics/Variables#XWayland|hl.config({ xwayland = { force_zero_scaling = true } })]]</code> option, which forces XWayland windows not to be scaled.
...
Keeping that in mind, we add the <code>[[hyprland/Configuring/Basics/Variables#XWayland|xwayland:create_abstract_socket]]</code> option. When the abstract socket is disabled, only the regular Unix domain socket will be created.
