...
> [!NOTE]
> [[Systemd start|uwsm]] users should avoid placing environment variables in the `hyprland.lua` file.
...
> [!NOTE]
> When referencing existing environment variables like `$XDG_RUNTIME_DIR`, use `os.getenv()`, eg.: `hl.env("SSH_AUTH_SOCK", os.getenv("XDG_RUNTIME_DIR").."/ssh-agent.socket")`
...
> [!NOTE]
> [[Systemd start|uwsm]] users don't need to explicitly set XDG environment variables, as uwsm sets them automatically.
