---
parent: "[[Hyprland Wiki]]"
prev: "[[Crashes and Bugs]]"
next: "[[Connect]]"
---
...
To force them to run in native Wayland mode, see [[Master Tutorial#Force apps to use Wayland|the Master Tutorial]].
...
For systems with limited hardware (Ex. iGPU, USB-C, USB Hubs) - Set env `AQ_NO_MODIFIERS` to `1` in your config. To diagnose if you have the exact problem above you can get a [[Crashes and Bugs#Debugging DRM issues|DRM log]] and look for
...
Check [[Screen Sharing|Screensharing]].
...
If you do not want to install a GTK settings editor, change the config files according to the [XDG specification (Arch Wiki link)](https://wiki.archlinux.org/title/Cursor_themes#Configuration). Make sure to also edit `~/.config/gtk-4.0/settings.ini` and `~/.gtkrc-2.0` if _not_ using a tool (like `nwg-look`).
...
Make sure you have a notification daemon running, for example `dunst`. You can autostart them in the config.
...
launched on start should fix all issues. Adjust the sleep durations to taste.
...
See [[xdg-desktop-portal-hyprland#Debugging|The XDPH Page]].
...
[[Workspace Rules#Smart gaps|Here]].
