---
parent: "[[Nix]]"
prev: "[[Hyprland on Other Distros]]"
next: "[[Cachix]]"
---
...
> [!WARNING]
> **Required:**
> - **NixOS Module:** enables critical components needed to run Hyprland properly.<br> Without this, you may have issues with missing session files in your Display Manager.
> 
> **Optional**:
> - **Home Manager module:** lets you configure Hyprland declaratively through Home Manager.
> - Configures Hyprland and adds it to your user's `$PATH`, but does not make certain system-level changes such as adding a desktop session file for your display manager.<br> This is handled by the NixOS module, once you enable it.
...
> [!info]- Flakes
>
>The following snippet of code tries to show how to bring the Hyprland flake from the flake input and use its packages with Home Manager. Feel free to make any adjustment for your setup.
>
...
> [!info]- Nix stable (with flake-compat)
>
>> [!WARNING]
...
>
>The following snippet of code tries to show how to bring the Hyprland flake from the flake input and use the package in the Home Manager option. Feel free to make any adjustment for your setup.
>
>```nix file:"home.nix"
>{pkgs, ...}: let
...
For examples on how to build Hyprland plugins using Nix, see the [[hyprland/Nix/Plugins|Nix/Plugins]] page.
...
If you're using the NixOS module with UWSM (`programs.hyprland.withUWSM = true`), you can [set environment variables][uwsm-env] like this:
