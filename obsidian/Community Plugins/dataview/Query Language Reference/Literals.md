Dataview query language _literals_ are **expressions** which represent constant values like a text (`"Science"`) or a number (`2021`). They can be used as part as [[obsidian/Community Plugins/dataview/Query Language Reference/Functions|functions]] or of [[Expressions|expressions like comparison]]. Some examples of [[Structure of a Query|Queries]] that use **literals**:
...
> [!note] Literals as field values
>
> Literals are only interpreted this way when used inside a Query, not when used as a meta data value. For possible values and their data types for fields, please refer to [[Data Types|Types of Metadata]].

### Dates

Whenever you use a [[Data Types#Date|field value in Date ISO format]], you'll need to compare these fields against date objects. Dataview provides some shorthands for common use cases like tomorrow, start of current week etc. Please note that `date()` is also a [[obsidian/Community Plugins/dataview/Query Language Reference/Functions#`date(any)`|function]], which can be called on **text** to extract dates.
...
Durations are representatives of a time span. You can either [[Data Types#Duration|define them directly]] or create them due to [[Data Types#Duration|calculating with dates]], and use these for i.e. comparisons.
