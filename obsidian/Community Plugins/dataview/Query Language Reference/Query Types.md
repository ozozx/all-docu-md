...
You can combine **every Query Type with all available [[Data Commands]]** to refine your result set. Read more about the interconnection between Query Types and Data Commands on [[obsidian/Community Plugins/dataview/Overview#How to Use Dataview|How to Use Dataview]] and the [[Structure of a Query|structure page]].
...
`LIST` queries output a bullet point list consisting of your file links or the group name, if you decided to [[Data Commands#GROUP BY|group]]. You can specify up to **one additional information** to output alongside your file or group information.

> [!note] Query Type `LIST`
>
...
but you can, of course, use [[Data Commands|data commands]] to restrict which pages you want to have listed:
...
> [!note] Changing the first column header name
>
...
>
> Curious about `file.folder` and `file.etags`? Learn more about [[Metadata on Pages|implicit fields on pages]].
...
> [!note] Custom headers with spaces
>
...
````
```dataview
TABLE default(finished, date(today)) - started AS "Played for",
file.folder AS Path,
...
> [!note] Calculations and expressions
>
> Learn more about the capability of computing expressions and calculations under [[Expressions|expressions]] and [[obsidian/Community Plugins/dataview/Query Language Reference/Functions|functions]].
...
> [!note] Renaming the first column in general
>
> If you want to rename the first column in all cases, change the name in Dataviews settings under Table Settings.

## TASK

The `TASK` Query outputs **an interactive list of all tasks in your vault** that match the given [[Data Commands|data commands]] (if any). `TASK` queries are special compared to the other Query Types because they do give back **Tasks as results and not pages**. This implies that all [[Data Commands|data commands]] operate on **Task level** and makes it possible to granularly filter your tasks i.e. for their status or meta data specified on the task itself.
...
You can use [[Data Commands|data commands]] like for all other Query Types. Data Commands are executed on task level, making [[Metadata on Tasks and Lists|implicit fields on tasks]] directly available.
...
> [!note] Counting tasks with subtask
>
...
The `CALENDAR` Query outputs a monthly based calendar where every result is depicted as a dot on it referring date. The `CALENDAR` is the only Query Type that requires an additional information. This additional information needs to be a [[Data Types#Date|date]] (or unset) on all queried pages.

> [!note] `CALENDAR` Query Type
>
