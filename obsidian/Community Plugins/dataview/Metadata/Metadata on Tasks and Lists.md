Just like pages, you can also add **fields** on list item and task level to bind it to a specific task as context. For this you need to use the [[Adding Metadata#Inline Fields|inline field syntax]]:
...
` ```dataview TASK WHERE completion = date("2021-08-22") ``` `
...
|`path`|Text|The full path of the file this task is in. Equals to `file.path` for [[Metadata on Pages\|pages]].|
...
With usage of the [[Metadata on Tasks and Lists#Field Shorthands|shorthand syntax]], following additional properties may be available:
...
If you're using a [[Query Types#TASK|TASK]] Query, your tasks are the top level information and can be used without any prefix:
...
This will give you back all the file links that have unfinished tasks inside. We get back a list of tasks on page level and thus need to use a [[obsidian/Community Plugins/dataview/Query Language Reference/Functions|list function]] to look at each element.