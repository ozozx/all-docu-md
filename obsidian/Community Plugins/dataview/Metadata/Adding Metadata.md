...
A metadata field is a pair of a **key** and a **value**. The _value_ of a field has a data type (more on that [[Data Types|here]]) that determines how this field will behave when querying it.
...
On **tasks or list items**, you will have YAML Frontmatter information available, but won't be able to add them to a specific list item. If you want to add metadata to one list item or task only, use [[Adding Metadata#Inline Fields|Inline Fields]].

### Frontmatter

Frontmatter is a common Markdown extension which allows for YAML metadata to be added to the top of a page. It is natively supported by Obsidian and explained in its [[Properties|official documentation]]. All YAML Frontmatter fields will be automatically available as Dataview fields.
```yaml
---
alias: "document"
...
- `alias` is a [[Data Types#Text|text]], because its wrapped in ""
- `last-reviewed` is a [[Data Types#Date|date]], because it follows the ISO date format
- `thoughts` is a [[Data Types#Object|object]] field, because it uses the YAML Frontmatter object syntax
...
**Keys with spaces** cannot be used in a query as-is. You have two possibilities here: Either use the sanitized name, that is always all lowercase and with dashes instead of spaces or use the **row** variable syntax. Find out more [[obsidian/Community Plugins/dataview/FAQ and Resources/Frequently Asked Questions|in the FAQ]].
...
>
> An exception to this are the [[Metadata on Tasks and Lists#Field Shorthands|shorthand syntax]] in Tasks. You can use shorthands without bracketing. Please mind though that this only counts for listed shorthands - every other field (if with emojis or not) need to use the `[key:​: value]` syntax.
