...
Dataview provides [[obsidian/Community Plugins/dataview/Query Language Reference/Functions|functions]] you can use to modify your metadata and allows you to write all sorts of complex queries. Specific functions need specific data types to work correctly. That means the data type of your field determines which functions you can use on these fields and how the functions behave. Furthermore, depending on the type, the output dataview renders can be different.
...
> [!error] Different rendering based on type
>
...
>
> `date1` is recognized as a **Date** while `date2` is a normal **Text** to dataview, that's why `date1` is parsed differently for you. Find out more on [[#Date|Dates below]].
...
gives you back all birthdays happening this month. Curious about `date(now)`? Read more about it under [[Literals#Dates|literals]].

> [!info] Displaying of date objects
>
> Dataview renders date objects in a human readable format, i.e. `3:15 PM - February 26, 2021`. You can adjust how this format looks like in Dataview's Setting under "General" with "Date Format" and "Date + Time Format". If you want to adjust the format in a specific query only, use the [[obsidian/Community Plugins/dataview/Query Language Reference/Functions#`dateformat(date datetime, string)`|dateformat function]].
...
Find the complete list of values that are recognized as a duration on [[Literals#Durations|literals]].

> [!note] Calculations with dates and durations
>
...
> ```
>
> Curious about `date(now)`? Read more about it under [[Literals#Dates|literals]].
