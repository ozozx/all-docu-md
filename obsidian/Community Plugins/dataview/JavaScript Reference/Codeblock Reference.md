...
Query methods allow you to query the Dataview index for page metadata; to render this data, use the methods in the [[#Render|render section]].
...
Take a single string argument, `source`, which is the same form as a [[Sources|query language source]]. Return a [[Data Arrays|data array]] of page objects, which are plain objects with all of the page fields as values.
...
As with `dv.pages`, but just returns a [[Data Arrays|data array]] of paths of pages that match the given source.
...
Convert a given value or array into a Dataview [[Data Arrays|data array]]. If the value is already a data array, returns it unchanged.
