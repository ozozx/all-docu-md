...
- A function. A variety of [[obsidian/Bases/Functions]] are built-in, and plugins can add additional functions.
...
Formula properties support basic arithmetic operators and a variety of built-in [[obsidian/Bases/Functions]]. In the future, plugins will be able to add functions for use in formulas.
...
The global [[obsidian/Bases/Functions|function]] `today()` can be used to get the current date, and `now()` can be used to get the current date with time.
...
See the [[obsidian/Bases/Functions|list of functions]] that can be used in formulas and [[Views|filters]].
...
- Many other [[obsidian/Bases/Functions|fields and functions]] are available on date objects.
...
- To construct a link, use the global `link` [[obsidian/Bases/Functions|function]], for example `link("filename")` or `link("https://obsidian.md")`.
